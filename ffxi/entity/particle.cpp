...
FFXIParticle::Init(lotus::Engine* engine, lotus::Scene* scene, std::weak_ptr<lotus::Entity> parent,
...
lotus::WorkerTask<> FFXIParticle::Load(std::shared_ptr<lotus::Entity> entity, lotus::Engine* engine,
                                       lotus::Scene* scene, std::weak_ptr<lotus::Entity> parent,
                                       FFXI::Generator* generator, std::shared_ptr<lotus::Model> model, size_t index)
{
    ...
    auto fpc =
        co_await FFXI::ParticleComponent::make_component(entity.get(), engine, *pc, *p, parent, generator, index);
    auto r = engine->config->renderer.RasterizationEnabled()
                 ? co_await lotus::Component::ParticleRasterComponent::make_component(entity.get(), engine, *pc, *p)
                 : nullptr;
    auto rt = engine->config->renderer.RaytraceEnabled()
                  ? co_await lotus::Component::ParticleRaytraceComponent::make_component(entity.get(), engine, *pc, *p)
                  : nullptr;
