...
enum class SampleFormat : uint32_t { ADPCM = 0, PCM = 1, ATRAC3 = 3 };
...
FFXI::Audio::Audio(lotus::Engine *_engine) : engine(_engine) {
  auto base_path =
      static_cast<FFXIConfig *>(engine->config.get())->ffxi.ffxi_install_path;
...
std::optional<lotus::AudioEngine::AudioInstance>
FFXI::Audio::playSound(uint32_t id) {
...
std::unique_ptr<SoLoud::AudioSource> FFXI::Audio::loadSound(uint32_t id) {
  for (const auto &path : sound_paths) {
    if (auto as = loadAudioSource(
            path / std::format("se/se{:03}/se{:06}.spw", id / 1000, id))) {
      return as;
    }
  }
...
std::unique_ptr<SoLoud::AudioSource> FFXI::Audio::loadMusic(uint32_t id) {
  for (const auto &path : sound_paths) {
    if (auto as = loadAudioSource(
            path / std::format("music/data/music{:03}.bgw", id))) {
      return as;
    }
  }
...
std::unique_ptr<SoLoud::AudioSource>
FFXI::Audio::loadAudioSource(std::filesystem::path path) {
  ...
  if (std::string((const char *)data.data(), 8).starts_with("SeWave")) {
    ...
    if (header->sample_format == SampleFormat::ADPCM) {
      return std::make_unique<ADPCM>(std::move(file), header->sample_blocks,
                                     header->blocksize, header->loop_start,
                                     header->channels, sample_rate);
    } else if (header->sample_format == SampleFormat::PCM) {
      return std::make_unique<PCM>(std::move(file), header->sample_blocks,
                                   header->blocksize, header->loop_start,
                                   header->channels, sample_rate);
    }
  } else if (std::string((const char *)data.data(), 8)
                 .starts_with("BGMStrea")) {
    data.resize(sizeof(BGMHeader));
    file.read((char *)data.data() + 8, sizeof(BGMHeader) - 8);
    BGMHeader *header = (BGMHeader *)(data.data());
    float sample_rate = (header->sample_rate_low + header->sample_rate_high);
    return std::make_unique<ADPCMStream>(std::move(file), header->sample_blocks,
                                         header->blocksize, header->loop_start,
                                         header->channels, sample_rate);
  }
  return nullptr;
